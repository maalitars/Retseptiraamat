...
public class Graafika extends Application {
    private static List<String> aadressid = new ArrayList<>();
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        Scene stseen = esimene_aken.tegemine();
                    ...
                    public void handle(MouseEvent event)  {
                        ...
                        if(valik1.isSelected()) {
                            ...
                            button.setOnMouseClicked(new EventHandler<MouseEvent>() {
                                @Override
                                public void handle(MouseEvent event) {
                                    String nimi = tekstivastus.getTekst().getCharacters().toString(); //retsepti avamine nime järgi
                                    if (!nimi.isEmpty()) {
                                        for (Retsept retsept : söögid) {
                                            if (retsept.getNimi().contains(nimi)){
                                                aadressid.add(retsept.getAadress());
                                                retsept.võta_netist();
                                            }
                                        }
                            ...
                            nupp1.setOnMouseClicked(new EventHandler<MouseEvent>() {
                                ...
                                public void handle(MouseEvent event) {
                                    if (val1.isSelected()) {
                                        ...
                                        aadressid.add(söögid.get(arv).getAadress());
                                        viimane_aken(primaryStage, stseen);
                                    }
                                    else {
                                        int arv = (int) Math.round(Math.random() * (söögid.size() - 1));
                            ...
                            button.setOnMouseClicked(new EventHandler<MouseEvent>() {
                                ...
                                public void handle(MouseEvent event) {
                                    ...
                                    if (!koostis.isEmpty()) {
                                        for (Retsept retsept : söögid) {
                                            if (retsept.getKoostisosad().contains(koostis)){
                                                aadressid.add(retsept.getAadress());
                                               retsept.võta_netist();
                                            }
                                        }
                                ...
                                public void handle(MouseEvent event) {
                                    ...
                                    if (val1.isSelected() || val2.isSelected()) {
                                        if(val1.isSelected()) {
                                            ...
                                            aadressid.add(söögid.get(i).getAadress());
                                        }
                                        else {
                                            int i = 1;
                                            söögid.get(söögid.size()-i).võta_netist();
                                            aadressid.add(söögid.get(söögid.size()-i).getAadress());
                                        }
                                            ...
                                            public void handle(MouseEvent event) {
                                                ...
                                                try{
                                                    if(val1.isSelected()){
                                                        ...
                                                        aadressid.add(söögid.get(a).getAadress());
                                                    }
                                                    else{
                                                        söögid.get(söögid.size()-a).võta_netist();
                                                        aadressid.add(söögid.get(söögid.size()-a).getAadress());
                                                    }
                                                }
    ...
    private static void viimane_aken(Stage primary, Scene stseen){
        ...
        viimane.getTeine().setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                failiKirjutaja(aadressid, new File("logi.txt"));
                lava.close();
                primary.close();
            }
        });
    ...
    private static void failiKirjutaja(List<String> veebiaadressid, File fail) {
        try (BufferedWriter br = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fail)))) {
            for (String aadress : veebiaadressid) {
                br.write("Retsepti URL: " + aadress );
                br.newLine();
            }
        }
        catch(IOException e ){
